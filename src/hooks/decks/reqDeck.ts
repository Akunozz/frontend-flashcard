...
export async function reqGetDeckById(id: string | number): Promise<IDeck> {
  const response = await fetch(`${API_BASE_URL}/decks/${id}`);
  if (!response.ok) throw new Error("Erro ao buscar deck");
  return await response.json();
}

export async function reqUpdateDeck(id: string | number, data: DeckUpdateDTO): Promise<IDeck> {
  const response = await fetch(`${API_BASE_URL}/decks/${id}` , {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
...
export async function reqDeleteDeck(id: string | number): Promise<void> {
  const response = await fetch(`${API_BASE_URL}/decks/${id}`, {
    method: "DELETE",
  });
...
export async function reqGetDecksByTurmaId(turmaId: string | number): Promise<IDeck[]> {
