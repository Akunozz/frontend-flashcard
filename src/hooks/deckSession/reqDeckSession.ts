...
export async function getCardsFromDeck(deckId: number): Promise<ICard[]> {
  const response = await fetch(
    `${API_BASE_URL}/deck-sessions/decks/${deckId}/cards`
  );
...
// POST /deck-sessions
export async function reqDeckSession(data: {
  studentId: string;
  deckId: number;
  reviews: { cardId: number; result: "CORRECT" | "INCORRECT" }[];
}): Promise<IDeckSession> {
